...
import type { usePedidos, PedidoConCliente, CrearPedidoData, DatosVentaEntrega, EstadoPedido } from '@/hooks/usePedidos'
...
// ── BANNER DE ALERTAS ─────────────────────────────────────────────────────────
function BannerAlertas({ alertas, t }: {
  alertas: ReturnType<typeof usePedidos>['alertas']
  t: Tema
}) {
        ...
        <div style={{ padding: '10px 14px', borderRadius: 11, background: '#fff1f2', border: '1px solid #fecdd3', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: '#dc2626' }}>
              {alertas.atrasados.length} pedido{alertas.atrasados.length > 1 ? 's' : ''} atrasado{alertas.atrasados.length > 1 ? 's' : ''}
            </span>
            <span style={{ fontSize: 11, color: '#9f1239', marginLeft: 6 }}>
              {alertas.atrasados.map(p => p.clientes?.nombre ?? 'Cliente').join(', ')}
            </span>
          </div>
        </div>
      )}
        ...
        <div style={{ padding: '10px 14px', borderRadius: 11, background: '#fffbeb', border: '1px solid #fde68a', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: '#d97706' }}>
              {alertas.hoy.length} entrega{alertas.hoy.length > 1 ? 's' : ''} para hoy
            </span>
            <span style={{ fontSize: 11, color: '#92400e', marginLeft: 6 }}>
              {alertas.hoy.map(p => p.clientes?.nombre ?? 'Cliente').join(', ')}
            </span>
          </div>
        </div>
      )}
...
// ── RESUMEN SEMANAL ───────────────────────────────────────────────────────────
function ResumenSemanal({ pedidos, t }: {
    ...
    <div style={{ margin: '0 20px', padding: '12px 16px', borderRadius: 13, background: t.surface, border: `1px solid ${t.border}`, boxShadow: t.shadow }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
        {[
          ...
          { label: 'Señas ya cobradas', value: formatPeso(señasCobradas), sub: 'anticipado' },
        ].map((k, i) => (
          <div key={i}>
    ...
    <div style={{
      ...
      >
        ...
        <div style={{
          ...
          background: dc.color, boxShadow: `0 0 6px ${dc.color}88`,
        }} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, flexShrink: 0 }}>
          <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 20, background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.border}` }}>
            {cfg.emoji} {cfg.label}
          </span>
          <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 20, background: dc.bg, color: dc.color, border: `1px solid ${dc.border}` }}>
            {formatFecha(pedido.fecha_entrega)} · {diasLabel(pedido.dias_restantes)}
          </span>
        </div>
      ...
      {expandido && (
        <div style={{ borderTop: `1px solid ${t.border}`, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {/* Notas */}
          {pedido.notas && (
            <div style={{ padding: '8px 10px', borderRadius: 9, background: t.surfaceAlt, border: `1px solid ${t.border}`, fontSize: 12, color: t.textMuted }}>
              📝 {pedido.notas}
            </div>
          )}
...
function ModalConfirmarEntrega({ pedido, t, dark, pedidosHook, onClose }: {
  ...
  t: Tema
  dark: boolean
  pedidosHook: ReturnType<typeof usePedidos>
  onClose: () => void
}) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ padding: '12px 14px', borderRadius: 11, background: t.surfaceAlt, border: `1px solid ${t.border}` }}>
        ...
        {monto > 0 && (
          <div style={{ fontSize: 15, fontWeight: 800, color: t.accent, fontFamily: 'monospace', marginTop: 6 }}>
            A cobrar: {formatPeso(monto)}
          </div>
        )}
        {toFloat(pedido.monto_seña) > 0 && (
          <div style={{ fontSize: 11, color: t.greenText, marginTop: 2 }}>
            ✓ Seña ya cobrada: {formatPeso(pedido.monto_seña)}
          </div>
        )}
      </div>
      ...
      <div style={{ fontSize: 13, fontWeight: 600, color: t.text }}>
        ¿Querés registrar la venta?
      </div>

      {/* Opciones de pago (solo si hay monto a cobrar) */}
      {monto > 0 && (
        <div>
          <label style={{ fontSize: 11, fontWeight: 700, color: t.textMuted, display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
            Forma de cobro
          </label>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {(['efectivo', 'transferencia', 'tarjeta', 'cuotas'] as const).map(tp => (
              <button key={tp} onClick={() => setTipoPago(tp)}
                style={{ padding: '9px 0', borderRadius: 10, border: `1.5px solid ${tipoPago === tp ? t.accent : t.border}`, background: tipoPago === tp ? (dark ? '#2a2218' : t.surfaceAlt) : t.surface, color: tipoPago === tp ? t.accent : t.textMuted, fontSize: 12, fontWeight: tipoPago === tp ? 700 : 400, cursor: 'pointer', transition: 'all 0.15s' }}>
          ...
          {tipoPago === 'cuotas' && (
            <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <span style={{ fontSize: 11, color: t.textMuted }}>
                → {formatPeso(monto / cantCuotas)} c/u
              </span>
            </div>
          )}
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <button onClick={onClose}
          style={{ width: '100%', padding: 9, borderRadius: 12, border: 'none', background: 'none', color: t.textFaint, fontSize: 11, cursor: 'pointer' }}>
          Cancelar
        </button>
      </div>
    </div>
...
function ModalNuevoPedido({ pedidosHook, t, dark, onClose }: {
  ...
  // Fecha mínima = mañana — calculada una sola vez en el client con useState
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        ...
        <div>
          <label style={labelStyle}>Seña <span style={{ fontWeight: 400, textTransform: 'none' }}>(opc.)</span></label>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <button onClick={handleGuardar} disabled={!puedeGuardar || pedidosHook.saving}
          style={{ flex: 2, padding: 12, borderRadius: 12, border: 'none', background: puedeGuardar ? t.accent : t.surfaceAlt, color: puedeGuardar ? t.accentText : t.textFaint, fontSize: 13, fontWeight: 800, cursor: puedeGuardar ? 'pointer' : 'not-allowed', transition: 'all 0.15s' }}>
...
function ListaPedidos({ pedidos: hook, t, dark, onEntregar }: {
  ...
  const filtrados = filtro === 'todos'
    ? hook.pedidos
    : hook.pedidos.filter(p => p.estado === filtro)
      ...
      <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 2 }}>
        {tabs.map(tab => (
          <button key={tab.key} onClick={() => setFiltro(tab.key)}
            style={{ padding: '6px 12px', borderRadius: 20, border: `1.5px solid ${filtro === tab.key ? t.accent : t.border}`, background: filtro === tab.key ? (dark ? '#2a2218' : t.surfaceAlt) : t.surface, color: filtro === tab.key ? t.accent : t.textMuted, fontSize: 11, fontWeight: filtro === tab.key ? 700 : 400, cursor: 'pointer', whiteSpace: 'nowrap' as const, transition: 'all 0.15s' }}>
            {tab.label}
          </button>
        ))}
      </div>
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: isMobile ? 80 : 20 }}>
        ...
        <div style={{ padding: '12px 0 0' }}>
          <BannerAlertas alertas={pedidos.alertas} t={t} />
        </div>

        {/* Resumen semanal */}
        <div style={{ padding: '12px 0 0' }}>
          <ResumenSemanal pedidos={pedidos.pedidos} t={t} />
        </div>
      ...
      {isMobile && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: t.navBg, backdropFilter: 'blur(16px)', borderTop: `1px solid ${t.border}`, padding: '10px 0 20px', display: 'flex', justifyContent: 'space-around', zIndex: 50 }}>
          {[['⊞','Inicio','/dashboard'],['↗','Ventas','/ventas'],['◎','Cobros','/cobranzas'],['📋','Pedidos','/pedidos'],['≋','Más','']].map(([icon,label,href]) => (
    ...
    <>
      ...
      {showNuevo && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: isMobile ? 'flex-end' : 'center', justifyContent: 'center', padding: isMobile ? 0 : 20 }}>
          <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: isMobile ? '20px 20px 0 0' : 20, padding: '24px 20px', width: '100%', maxWidth: isMobile ? '100%' : 480, maxHeight: isMobile ? '94vh' : '90vh', overflowY: 'auto', boxShadow: t.shadowMd }}>
