...
const formatPeso = (n: string | number | null | undefined) => {
  const num = toFloat(n)
  // Formato manual para evitar hydration mismatch (toLocaleString difiere entre server/client)
...
function getSemaforoCuota(cuota: Cuota): 'pagada' | 'vencida' | 'hoy' | 'pendiente' {
  ...
  // Parseo local para evitar desfase UTC (new Date("YYYY-MM-DD") interpreta como UTC)
  ...
  if (venc.getTime() === ahora.getTime()) return 'hoy'   // exactamente hoy → recorrido, badge amarillo
  if (venc < ahora) return 'vencida'                      // días anteriores → badge rojo
  return 'pendiente'
}
...
function Sk({ h=48, t }: { h?: number; t: Tema }) {
  return <div style={{ height:h, borderRadius:12, background:t.surfaceAlt, overflow:'hidden', position:'relative' }}>
...
function ModalCobro({ cuota, clienteNombre, onConfirm, onCancel, t, dark }: {
...
// ── Card de cobranza ──────────────────────────────────────────────────────────
function CardCobranza({ cobranza, t, dark, onCobrar, onMarcarProblematico, onEditarFecha, onComprobante, onQuitarProblematico }: {
  cobranza: CobranzaConDetalle
  t: Tema; dark: boolean
  onCobrar: (cuota: Cuota, clienteNombre: string) => void
  ...
  // Normalizado: ya no es moroso pero tiene historial previo de problemas
  const tieneHistorial = !esMoroso && cliente?.motivo_moroso?.startsWith('[NORMALIZADO]')

  return (
    <div style={{ background:t.surface, border:`1px solid ${vencidas>0 ? t.redBorder : t.border}`, borderRadius:15, padding:'14px 16px', boxShadow:t.shadow, transition:'border 0.2s' }}>

      {/* Header de la card */}
      <div style={{ display:'flex', alignItems:'flex-start', gap:12 }}>

        {/* Avatar + moroso / historial */}
        <div style={{ position:'relative', flexShrink:0 }}>
          ...
          {esMoroso && (
            <div style={{ position:'absolute', top:-4, right:-4, width:16, height:16, borderRadius:'50%', background:t.redNum, display:'flex', alignItems:'center', justifyContent:'center', fontSize:9 }}>🚨</div>
          )}
          {tieneHistorial && (
            <div style={{ position:'absolute', top:-4, right:-4, width:16, height:16, borderRadius:'50%', background:t.amberSub, display:'flex', alignItems:'center', justifyContent:'center', fontSize:9 }}>⚠️</div>
          )}
        </div>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {esMoroso && cliente?.motivo_moroso && (
            <div style={{ fontSize:10, color:t.redNum, marginTop:3, fontStyle:'italic' }}>
              ⚠️ {cliente.motivo_moroso}
            </div>
          )}
      ...
      <div style={{ marginTop:12 }}>
        ...
        <div style={{ height:5, borderRadius:3, background:t.border, overflow:'hidden' }}>
          <div style={{ height:'100%', width:`${progreso}%`, background:vencidas>0?t.redNum:progreso===100?'#4ade80':t.accent, borderRadius:3, transition:'width 0.4s ease' }} />
        </div>
      </div>
      ...
      <div style={{ display:'flex', gap:8, marginTop:12 }}>
        ...
        <button
          onClick={() => onComprobante(cobranza)}
          title="Descargar comprobante de cuotas"
          style={{ padding:'7px 10px', borderRadius:9, border:`1px solid ${t.border}`, background:t.surfaceAlt, color:t.textMuted, fontSize:13, fontWeight:600, cursor:'pointer', whiteSpace:'nowrap' as const }}>
          ⬇ PDF
        </button>
        {!esMoroso ? (
      ...
      {/* Cuotas expandidas */}
      {expandida && (
        <PanelCuotas cobranza={cobranza} t={t} dark={dark} onCobrar={onCobrar} onEditarFecha={onEditarFecha} />
      )}
    </div>
  )
}
...
function ModalNuevaCobranza({ cobranzas, t, dark, onClose }: {
  cobranzas: ReturnType<typeof useCobranzas>
  t: Tema; dark: boolean
  onClose: () => void
}) {
  ...
  const handleGuardar = async () => {
    ...
    try {
      await cobranzas.crearCobranzaManual({
  ...
  if (exito) return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:240, gap:12 }}>
      ...
      <div style={{ fontSize:16, fontWeight:800, color:t.text }}>Cobranza creada</div>
    </div>
  )
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      ...
      <div>
        ...
        <input value={descripcion} onChange={e=>setDescripcion(e.target.value)} placeholder="Ej: Préstamo personal, Fiado mercadería..." style={inputStyle} />
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
        <div>
          ...
          <input type="number" value={monto} onChange={e=>setMonto(e.target.value)} placeholder="$0" style={inputStyle} />
        </div>
        <div>
          ...
          <div style={{ display:'flex', gap:6 }}>
            <input
              ...
              placeholder="Ej: 3"
              style={{ width:'100%', padding:'10px 12px', borderRadius:10, border:`1.5px solid ${t.border}`, background:t.bg, color:t.text, fontSize:13, fontFamily:'inherit', outline:'none', boxSizing:'border-box' as const }}
            />
          </div>
        </div>
      </div>
...
export function CobranzasView({ usuario, cobranzas }: CobranzasViewProps) {
  ...
  // Filtros
  // Cobranzas activas: excluye clientes problemáticos que no pagaron NINGUNA cuota
  // (esos van directo a la tab de Problemáticos)
  const cobranzasActivas = cobranzas.cobranzas.filter(c => {
    if (c.estado !== 'activa' && c.estado !== 'vencida') return false
    if (!c.clientes?.es_moroso) return true  // no moroso → siempre en activas
    // Es moroso: mostrar en activas SOLO si pagó al menos una cuota
    const cuotas = c.cuotas ?? []
    const pagaAlguna = cuotas.some(q => q.estado === 'pagada')
    return pagaAlguna
  })
  ...
  // Cuotas que vencen hoy agrupadas por cobranza
  const cobranzasConCuotasHoy = cobranzasActivas.filter(c =>
    (c.cuotas ?? []).some(q =>
      q.fecha_vencimiento?.slice(0, 10) === hoy && q.estado === 'pendiente'
    )
  )
  ...
  // Sincronizar recorrido cuando cargan las cobranzas
  useEffect(() => {
    ...
    setRecorrido(prev => {
      ...
      const actualizadas = prev
        .filter(c => cobranzasConCuotasHoy.some(x => x.id === c.id))
        .map(c => cobranzasConCuotasHoy.find(x => x.id === c.id) ?? c)
  ...
  // Touch handlers para mobile — usa el handle ≡ como punto de agarre
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (touch.clientY >= rect.top && touch.clientY <= rect.bottom) {
        if (touchToIdx.current !== i) {
          touchToIdx.current = i
          setDragOverIdx(i)
        }
        break
      }
  ...
  const handleTouchEnd = () => {
    ...
    touchToIdx.current   = null
    setDragIdx(null)
    setDragOverIdx(null)
  }

  // editarFechaCuota — llama al hook
  ...
  const sidebar = (
    <Sidebar activo="cobranzas" usuario={usuario} dark={dark} setDark={setDark} t={t} />
  )
    ...
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
      ...
      <div style={{ height:54, background:t.surface, borderBottom:`1px solid ${t.border}`, display:'flex', alignItems:'center', padding:'0 20px', flexShrink:0 }}>
        ...
        <button onClick={()=>setShowNueva(true)}
          style={{ marginLeft:'auto', padding:'8px 14px', borderRadius:10, border:'none', background:t.accent, color:t.accentText, fontSize:12, fontWeight:800, cursor:'pointer' }}>
          ＋ Nueva
        </button>
      </div>
        ...
        {tab === 'recorrido' && (
          <div ref={listRef} style={{ display:'flex', flexDirection:'column', gap:10 }}>
            <div style={{ padding:'4px 0 8px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              ...
              {recorrido.length > 0 && (() => {
                const totalDia = recorrido.reduce((s, c) => {
                  const q = (c.cuotas ?? []).find(cq => cq.fecha_vencimiento?.slice(0, 10) === hoy)
                  return s + toFloat(q?.monto ?? 0)
                }, 0)
                const totalCobrado = recorrido.reduce((s, c) => {
                  const q = (c.cuotas ?? []).find(cq => cq.fecha_vencimiento?.slice(0, 10) === hoy)
                  const cobrado = cobradosHoy.has(c.id) || q?.estado === 'pagada'
                  return s + (cobrado ? toFloat(q?.monto ?? 0) : 0)
                }, 0)
                return (
            ...
            {recorrido.length === 0 ? (
              <div style={{ textAlign:'center', padding:'40px 20px', color:t.textFaint }}>
                ...
                <div style={{ fontSize:11, color:t.textFaint, marginTop:4 }}>Las cuotas que venzan hoy aparecen acá automáticamente</div>
              </div>
            ) : recorrido.map((c, idx) => {
              // Buscar cuota de hoy específicamente
              const cuotaHoy = (c.cuotas ?? []).find(q =>
                q.fecha_vencimiento?.slice(0, 10) === hoy && q.estado === 'pendiente'
              )
              // Fallback: si ya se cobró en esta sesión, buscar la cuota pagada de hoy para mostrar el monto
              const cuotaHoyPagada = cobradosHoy.has(c.id)
                ? (c.cuotas ?? []).find(q => q.fecha_vencimiento?.slice(0, 10) === hoy)
                : undefined
              const cuotaMostrar = cuotaHoy ?? cuotaHoyPagada
              // cobrado = se marcó en esta sesión via cobradosHoy
              ...
              return (
                <div
                  key={c.id}
                  draggable={!cobrado}
                  onDragStart={() => !cobrado && handleDragStart(idx)}
                  ...
                  style={{
                    ...
                    background: cobrado ? t.green : isDragOver ? t.surfaceAlt : t.surface,
                    border:`1.5px solid ${cobrado ? t.greenBorder : isDragOver ? t.accent : t.border}`,
                    boxShadow: isDragging ? t.shadowMd : t.shadow,
                    opacity: isDragging ? 0.5 : 1,
                    cursor: cobrado ? 'default' : 'grab',
                    transition:'all 0.2s ease',
                    transform: isDragOver ? 'scale(1.01)' : 'none',
                  }}
                >
                  {/* Número de parada / check si cobrado */}
                  <div style={{ width:28, height:28, borderRadius:8, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize: cobrado ? 16 : 12, fontWeight:800,
                    background: cobrado ? t.greenBorder : t.accent,
                    color: cobrado ? t.greenText : t.accentText,
                  }}>
                    {cobrado ? '✓' : idx + 1}
                  </div>
                  {/* Handle drag — solo si no cobrado */}
                  {!cobrado && (
                    <div
                  ...
                  {/* Info cliente */}
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontSize:13, fontWeight:700, color: cobrado ? t.greenText : t.text, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis',
                      textDecoration: cobrado ? 'line-through' : 'none', opacity: cobrado ? 0.8 : 1 }}>
                      {cliente?.nombre ?? 'Cliente'}
                    </div>
                    <div style={{ fontSize:10, color: cobrado ? t.greenText : t.textFaint, marginTop:2, opacity: cobrado ? 0.7 : 1 }}>
                      {cobrado ? '¡Cobrado!' : (c.descripcion ?? 'Sin descripción')}
                      {!cobrado && cliente?.telefono ? ` · ${cliente.telefono}` : ''}
                    </div>
                  </div>
                  {/* Monto cuota */}
                  <div style={{ textAlign:'right', flexShrink:0 }}>
                    <div style={{ fontSize:14, fontWeight:800, fontFamily:'monospace',
                      color: cobrado ? t.greenText : t.text,
                      textDecoration: cobrado ? 'line-through' : 'none', opacity: cobrado ? 0.7 : 1 }}>
                      {formatPeso(cuotaMostrar?.monto ?? 0)}
                    </div>
                    <div style={{ fontSize:9, color: cobrado ? t.greenText : t.textFaint }}>
                      Cuota {cuotaMostrar?.numero_cuota ?? '?'}/{c.cant_cuotas}
                    </div>
                  </div>
                  {/* Botón cobrar o badge cobrado */}
                  {cobrado ? (
                    ...
                    <button
                      onClick={() => setModalCobro({ cuota: cuotaHoy, cliente: cliente?.nombre ?? 'Cliente' })}
            ...
            ? (
              <div style={{ textAlign:'center', padding:'40px 20px', color:t.textFaint }}>
                ...
                <div style={{ fontSize:14, fontWeight:600, color:t.textMuted }}>
                  {tab==='activas' ? 'Sin cobranzas activas' : 'Sin clientes problemáticos'}
                </div>
              </div>
            )
            : lista.map(c=>(
  ...
  return (
    <>
      ...
      {modalCobro && (
        <ModalCobro
          cuota={modalCobro.cuota}
          clienteNombre={modalCobro.cliente}
          onConfirm={async () => {
            const cobranzaId = cobranzas.cobranzas.find(cb =>
              (cb.cuotas ?? []).some(q => q.id === modalCobro.cuota.id)
            )?.id
      ...
      {modalProb && (
        <ModalProblematico
          ...
          onConfirm={async (motivo, detalle) => {
            await cobranzas.marcarClienteProblematico(modalProb.id, motivo, detalle)
            setModalProb(null)
          }}
          onCancel={() => setModalProb(null)}
          t={t}
        />
      )}
